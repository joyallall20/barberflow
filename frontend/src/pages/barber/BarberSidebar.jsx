...
export const BARBER_NAV_GROUPS = [
  {
    label: "Overview",
    items: [
      {
...
const BarberSidebar = ({ railOpen, onCloseRail }) => {
  ...
  const barberName = mongoUser?.name || "—";
  const barberEmail = mongoUser?.email || "—";

  /* ---------- Shared nav list renderer ---------- */
  const renderNav = () =>
    BARBER_NAV_GROUPS.map((group) => (
  ...
  return (
    <>
      ...
      <aside className="hidden w-64 flex-shrink-0 border-r border-white/10 bg-[#0f0e0d] lg:block">
        ...
        <nav className="px-3 py-6">{renderNav()}</nav>
      ...
      {railOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button
            type="button"
            aria-label="Close barber navigation"
