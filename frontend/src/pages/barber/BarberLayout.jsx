...
import BarberSidebar, { BARBER_NAV_GROUPS } from "./BarberSidebar";

const BarberLayout = () => {
  const location = useLocation();

  const currentNav =
    BARBER_NAV_GROUPS.flatMap((group) =>
  ...
  // Soft guard — only barbers belong here.
  if (!loading && !bootstrapping && role && role !== "barber") {
    const home =
          ...
          <header className="flex items-center justify-between border-b border-white/10 px-6 py-5 lg:px-10">
            <div className="flex items-center gap-4">
              <button
                ...
                aria-label="Open barber navigation"
